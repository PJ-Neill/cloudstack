  ...
  return (
    <div className="pt-24 pb-24">
      ...
      <div className="max-w-3xl mx-auto px-6 mb-10">
        ...
        <h1 style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(28px, 4vw, 44px)', fontWeight: 800, letterSpacing: '-0.03em', lineHeight: 1.15, color: 'var(--text-primary)', marginBottom: '16px' }}>{post.title}</h1>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p style={{ fontSize: '12px', color: 'var(--text-subtle)', fontFamily: 'var(--font-mono)' }}>{post.date} · {post.readTime}</p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-6 mb-12">
        <div className="relative w-full rounded-2xl overflow-hidden" style={{ height: '280px', background: 'var(--surface-card)' }}>
  ...
  --parameters @params.json`}
      ...
      {related.length > 0 && (
        <div className="max-w-3xl mx-auto px-6 mt-16">
          <h2 style={{ fontFamily: 'var(--font-display)', fontSize: '20px', fontWeight: 700, color: 'var(--text-primary)', marginBottom: '16px' }}>Related posts</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {related.map(p => (
              <Link key={p.slug} href={`/blog/${p.slug}`} className="group card-hover rounded-xl overflow-hidden flex flex-col" style={{ border: '1px solid var(--border-card)', background: 'var(--surface-card)' }}>
                ...
                <div className="p-4">
                  <h3 style={{ fontSize: '14px', fontWeight: 500, color: 'var(--text-secondary)', lineHeight: 1.4 }}>{p.title}</h3>
                  <p style={{ fontSize: '11px', color: 'var(--text-subtle)', marginTop: '8px', fontFamily: 'var(--font-mono)' }}>{p.date} · {p.readTime}</p>
                </div>
              </Link>
            ))}
          </div>
